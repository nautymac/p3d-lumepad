...
## 사진 보기, 자막 크기, SBS 수렴 (2026-09-05)

세 가지를 한 번에 다뤘다. 셋 다 "Lume Pad 2 에서는 화면을 CNSDK 가 그린다" 는
사실에서 갈라져 나온 문제라 따로 볼 이유가 없었다.

### 사진도 같은 화면에서 본다

3D 로 내보내는 길이 영상과 완전히 같다 — 레터박스, SBS/TB 크롭, 수렴, 위빙,
Leia 신경망 변환까지. 다른 것은 프레임을 만들어 넣는 쪽뿐이라 `PhotoEngine` 하나만
`VideoEngine` 자리에 갈아 끼웠다. 뷰는 여전히 "SurfaceTexture 로 프레임이 들어온다"
만 안다.

정지 화면인데도 계속 흘려보낸다. 이유가 둘이다.

- Leia 변환기는 들어온 프레임 수로 시간을 세고 RGB 를 몇 프레임 늦춰 깊이와 맞춘다.
  한 장만 넣으면 그 지연 구간을 못 빠져나온다.
- 우리 GL 도 "새 프레임이 있을 때만" 밖으로 내보낸다 (`Stereo3DView` 의 `fresh`).

다만 계속 최고 속도로 보낼 이유는 없어서 처음 3초만 15fps 로 넣고 그 뒤로는
2.5fps 로 유지만 한다. 복사는 `lockHardwareCanvas` 로 GPU 에 맡긴다.

**배치는 띄우기 전에 확정한다.** 영상은 판별이 재생 중에 끝나서 결과가 나오면
갈아끼우는데 그 교체가 화면에 보인다. 사진은 `inJustDecodeBounds` 로 원본 해상도를
즉시, 정확히 읽을 수 있어서 그럴 이유가 없다. `surfaceReady` / `panelReady` /
`formatReady` 세 조건이 다 설 때까지 미룬다.

### 해상도가 결정적이면 픽셀을 보지 않는다

기기의 게임 스크린샷 1883장을 표본으로 재보니 전부 full-SBS 였다.

| 해상도 | 장수(표본 36) | 눈 하나 |
|---|---|---|
| 3344x940 | 26 | 1672x940 = 1.779 |
| 3840x1080 | 6 | 1920x1080 = 1.778 |
| 5120x1440 | 4 | 2560x1440 = 1.778 |

그런데 픽셀 판별은 이것들을 **2D 라고 답했다**. Aragami 스크린샷(진짜 full-SBS)이
좌우차/대비 0.648, 상하차/대비 1.131 로 나와 문턱(0.38)도 배수(2.0)도 못 넘겼다.
3D Vision 계열이 영화보다 시차를 훨씬 크게 주기 때문이다.

3.56:1 짜리 프레임은 2D 일 수 없다. 그래서 `fromAspect` 가 답을 내면 픽셀 투표를
아예 건너뛴다. 영상 쪽 `detect()` 에도 같은 규칙을 넣었다.

### 목록은 폴더 단위로

사진 1883장을 한 줄로 늘어놓으면 못 쓴다. 영상도 이 기기에서는 내장과 SD 양쪽
여섯 폴더에 흩어져 있다. 그래서 목록을 폴더 → 파일 두 단계로 바꿨다
(`MediaLibrary`). 자주 여는 폴더는 길게 눌러 위로 고정한다.

묶는 기준은 MediaStore 의 `BUCKET_DISPLAY_NAME` 이 **아니라** `DATA` 컬럼(실제
경로)의 부모 디렉터리다. 버킷 이름은 맨 끝 폴더 이름일 뿐이라 서로 다른 위치의 같은
이름이 한 덩어리로 합쳐진다. 반대로 여는 것은 계속 `content://` 로 한다 —
targetSdk 24 이상에서 `file://` 를 인텐트에 실으면 StrictMode 가
`FileUriExposedException` 을 던진다.

### 자막이 눈곱만 했던 이유

자막 비트맵을 `glView` 크기로 그리고 있었다. ProMa 에서는 그 뷰가 화면 전체지만,
**Lume Pad 2 에서는 1x1 이다** — 화면은 CNSDK 뷰가 차지하고 우리 GLSurfaceView 는
GL 컨텍스트를 얻으려고 깔아둔 것뿐이다. 그래서 글자 18px, 폭 64px 짜리 비트맵이
만들어지고 한 줄에 한두 글자씩 끊겼다 (코랄라인·스파이더맨에서 확인).

기준을 "눈 하나" 로 바꿨다 (`Stereo3DView.eyeWidthPx/eyeHeightPx`). ProMa 는
화면 전체(2560x1600), Lume Pad 2 는 CNSDK 로 넘기는 프레임의 반쪽(1920x1200).
둘 다 화면 높이의 4.2% 라는 같은 규칙이 선다.

```
자막 기준 상자 1920x1200 -> 비트맵 1728x87 (배율 1.0)
```

### 수렴 보정 — 슬라이더가 이 기기에서 아무 일도 안 하고 있었다

값을 인터레이스 셰이더(`frag3D.sh` 의 `perOffset`)에만 넘기고 있었는데, 그 단계는
ProMa 에만 있다. Lume Pad 2 는 위빙을 CNSDK 가 하므로 슬라이더를 끝까지 밀어도
화면이 그대로였다.

좌/우 뷰를 FBO 에 그릴 때 **뷰포트를 반씩 반대로 밀고 가위질(scissor)** 하는 것으로
옮겼다. 두 패널에서 똑같이 동작하고, 밀려서 드러난 자리가 옆 눈이 아니라 검은색이
된다 (셰이더 쪽은 옆 눈의 그림을 끌어다 썼다 — 원본에도 그걸 검게 칠하려던 코드가
주석으로 남아 있다).

눈금도 화면 픽셀로 바꿨다. 게임 SBS 의 시차는 만들 때 쓰던 해상도의 픽셀 수로
굳어 있어서, 눈당 1672px 짜리를 눈 상자 1920px 에 채우면 1.15배로 커지고
2560px 짜리를 넣으면 0.75배로 준다. 소스 픽셀로 값을 잡으면 소스가 바뀔 때마다
뜻이 달라진다.

### 재보니 문제가 생각보다 컸다

스크린샷 36장의 좌우 절반이 **통째로** 어긋나 있었다 — 1920px 눈 기준 20px 부터
224px(눈 폭의 11.7%)까지. 정작 장면 자체의 깊이 폭은 28~184px 로 그보다 훨씬 작다.
24인치 모니터에서 만든 값을 태블릿에 그대로 올리니 두 눈이 모을 수 있는 한계를
넘는다. 사용자가 말한 "게임 SBS 를 태블릿 폭으로 줄이면 수렴이 안 맞는다" 가 이것이다.

측정은 두 단계다 (`Disparity`).

1. 전체 겹침으로 큰 오프셋 `d0` 를 잡는다 — "이 소스가 통째로 밀린 양".
2. `d0` 근처에서만 블록 매칭해 장면의 깊이 폭을 구한다.

한 단계로 ±8% 만 훑었을 때는 표본 대부분이 탐색 범위 끝에 붙어 값이 전혀 못 미더웠다.
실제 시차가 범위 밖이었기 때문이다.

**자동 규칙은 "장면 중심을 화면에"** 로 했다. 처음에는 "가장 앞을 화면에" 로 두려
했는데, 어느 쪽이 앞인지 확정할 수가 없었다. 아래쪽이 가깝다는 상식적인 단서로
재보니 36장 중 31장이 거꾸로 나왔는데 — 3D Vision 픽스가 HUD 를 화면 깊이에
고정해 두는 일이 많고 그 HUD 가 화면 아래쪽에 몰려 있다 — 좌우가 뒤바뀐 것인지
HUD 탓인지 가릴 수가 없었다. 중심을 화면에 놓으면 깊이 폭의 절반이 앞, 절반이
뒤로 갈려서 어느 해석이든 편한 범위에 들어온다.

### 확인한 방법

수렴이 실제로 걸리는지는 눈으로만 봐서는 알 수 없어서 `--ei conv N` 로 값을 고정하고
화면을 캡처해 열별 밝기를 쟀다. 밀려서 드러난 검은 자리가 가장자리 밝기로 나타난다.
...
디버그 인텐트: `--ez photo true`, `--es folder <경로>`, `--ei conv N`,
`--ez autoconv true`.
